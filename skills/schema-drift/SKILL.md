...
2. Collect expected schema from the transformation and actual input/target schemas via `dbx_inspect` action `table`. Record capture time; current metadata does not prove the schema at failure time. Where available, use approved SQL to inspect Delta history and historical versions.
