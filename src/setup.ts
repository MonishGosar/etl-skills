...
type Harness = "codex" | "claude" | "deepseek" | "pi" | "all";

const choices: Array<{ key: string; harness: Harness; label: string }> = [
...
async function installSkills(packageRoot: string, target: string, force: boolean) {
  ...
  for (const name of (await readdir(source)).sort()) {
    ...
    if (await exists(to) && !force) continue;
    await cp(from, to, { recursive: true, force, errorOnExist: !force });
  }
}

async function configureCodex(projectRoot: string, force: boolean) {
  ...
  if (current.includes("[mcp_servers.etl]")) {
    if (!force) return "kept existing .codex/config.toml ETL server";
...
async function configureClaude(projectRoot: string, force: boolean) {
  const path = resolve(projectRoot, ".mcp.json");
  const current = await exists(path) ? JSON.parse(await readFile(path, "utf8")) : {};
  current.mcpServers ??= {};
  if (current.mcpServers.etl && !force) return "kept existing .mcp.json ETL server";
  current.mcpServers.etl = claudeServer;
  await writeFile(path, `${JSON.stringify(current, null, 2)}\n`, "utf8");
  return "configured .mcp.json";
}

async function configureDeepSeek(projectRoot: string, force: boolean) {
  const directory = resolve(projectRoot, ".etl-agent");
  const path = resolve(directory, "deepseek.cordis.yml");
  await mkdir(directory, { recursive: true });
  if (await exists(path) && !force) return "kept existing .etl-agent/deepseek.cordis.yml";
...
for (let index = 0; index < args.length; index += 1) {
  ...
  else if (args[index] === "--harness" && args[index + 1]) harness = args[++index] as Harness;
  else if (args[index] === "--target" && args[index + 1]) projectRoot = resolve(args[++index]);
  else usage();
}
if (harness && !choices.some(choice => choice.harness === harness)) usage();

if (!harness) {
  if (!process.stdin.isTTY) usage();
  process.stdout.write("\nETL Agent Tools setup\n\nChoose your coding harness:\n");
  for (const choice of choices) process.stdout.write(`  ${choice.key}. ${choice.label}\n`);
...
const selected: Harness[] = harness === "all" ? ["codex", "claude", "deepseek", "pi"] : [harness];
...
if (selected.includes("pi")) changes.push("prepared the workspace profile for Pi");
...
for (const change of [...new Set(changes)]) process.stdout.write(`  ✓ ${change}\n`);
...
if (selected.includes("codex")) process.stdout.write("Verify Codex: codex mcp list\n");
if (selected.includes("claude")) process.stdout.write("Verify Claude Code: start claude, then run /mcp\n");
if (selected.includes("deepseek")) process.stdout.write("DeepSeek: merge .etl-agent/deepseek.cordis.yml into the active Cordis patch, then start the harness\n");
if (selected.includes("pi")) process.stdout.write("Install for Pi once: pi install npm:etl-agent-tools@0.2.0-beta.1\nThen start Pi: pi\n");
