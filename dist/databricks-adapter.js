...
import { numericId, required } from "./client.js";
export class DatabricksAdapter {
    ...
    async approve(options, operation, operationClass, payload) {
    ...
    async run(request, options) {
        ...
        if (token && token.length > 64)
            throw new Error("idempotency_token must be at most 64 characters.");
        const body = request.action === "start"
    ...
    async repair(request, options) {
        required(request.evidence, "evidence");
        if (!request.tasks.length || request.tasks.some(task => !task.trim()))
